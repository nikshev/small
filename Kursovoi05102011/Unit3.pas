...
begin
  ...
  if ADOConnection1.Connected then
   begin
    ...
    ADOQueryGroup.Open;
    RecCountG:=ADOQueryGroup.RecordCount;
    for i := 1 to RecCountG do   //Блок №16 (Функция построения дерева)
     begin
      GroupName:=ADOQueryGroup.Fields[0].AsString;  //Блок №17  (Функция построения дерева)
      ADOQueryGroup.Next;
      TreeView1.Items.AddChild(TreeView1.Items[0].Item[1],GroupName); //Блок №18  (Функция построения дерева)
      ADOQueryInvNum.Close;
      ADOQueryInvNum.SQL.Clear;
      ADOQueryInvNum.SQL.Add('SELECT Sredstva.InvNum '+
                            ' FROM Sredstva where NameSredstva='''+ GroupName+''''+
      ...
      ADOQueryInvNum.Open;
      RecCountI:=ADOQueryInvNum.RecordCount;
      for k := 1 to RecCountI do               //Блок №19  (Функция построения дерева)
       begin
        InvNum:=ADOQueryInvNum.Fields[0].AsString;    //Блок №20  (Функция построения дерева)
        ADOQueryInvNum.Next;
        TreeView1.Items.AddChild(TreeView1.Items[0].Item[1].Item[i-1],InvNum);   //Блок №21  (Функция построения дерева)
        ADOQueryParameters.Close;
        ADOQueryParameters.SQL.Clear;
        ADOQueryParameters.SQL.Add('SELECT Sredstva.Fio,Sredstva.Iznos,'+
                                   ' Sredstva.Rashod, Sredstva.DateB, Sredstva.Price '+
        ...
        ADOQueryParameters.Open;
        RecCountP:=ADOQueryParameters.RecordCount;
        for j := 1 to RecCountP do    //Блок №22  (Функция построения дерева)
         begin
          //Блок №23  (Функция построения дерева)
           Sotr:=ADOQueryParameters.Fields[0].AsString;
           Iznos:=ADOQueryParameters.Fields[1].AsFloat;
           Rashod:=ADOQueryParameters.Fields[2].AsInteger;
           DateB:=ADOQueryParameters.Fields[3].AsString;
           Price:=ADOQueryParameters.Fields[4].AsFloat;
           TreeView1.Items.AddChild(TreeView1.Items[0].Item[1].Item[i-1].Item[k-1],'Сотрудник: '+Sotr); //Блок №24 (Функция построения дерева)
           TreeView1.Items.AddChild(TreeView1.Items[0].Item[1].Item[i-1].Item[k-1],'Износ,%: '+FloatToStrF(Iznos,ffFixed,8,2));//Блок №25  (Функция построения дерева)
           TreeView1.Items.AddChild(TreeView1.Items[0].Item[1].Item[i-1].Item[k-1],'Расход: '+IntToStr(Rashod));//Блок №26  (Функция построения дерева)
           TreeView1.Items.AddChild(TreeView1.Items[0].Item[1].Item[i-1].Item[k-1],'Дата приобр.: '+DateB); //Блок №27  (Функция построения дерева)
           TreeView1.Items.AddChild(TreeView1.Items[0].Item[1].Item[i-1].Item[k-1],'Цена: '+FloatToStrF(Price,ffFixed,8,2)); //Блок №28  (Функция построения дерева)
           ADOQueryParameters.Next;
         end;
         ADOQueryParameters.Close;
       end;
